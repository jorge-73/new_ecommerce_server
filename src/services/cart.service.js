...
import { sendEmailPurchase } from "../libs/nodemailer/mailer.js";
...
export const purchaseService = async (req, res) => {
  ...
  for (const productInfo of cart.products) {
    const product = await ProductService.findById(productInfo.product._id);
  ...
  if (productsToPurchase.length > 0) {
    ...
    return res.sendUserError("No products were purchased.");
  }
};
